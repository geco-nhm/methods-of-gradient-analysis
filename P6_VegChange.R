...
gnmdstot <- read_xls("P6_OppkuvenGNMDS.xls", sheet = "BASIC") %>% as.data.frame() # Importing spreadsheet BASIC from data file P6OppkuvenGNMDS.xls
sp.change <- read_xls("P6_OppkuvenGNMDS.xls", sheet = "Species change") %>% as.data.frame() # Importing species data
...
baseline_plot <- ggplot() + coord_fixed(ratio = 1) + theme_bw() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
  
# Plot GNMDS (with plot ID labels)
baseline_plot + geom_text(data = gnmdstot, aes(x = GNMDS1, y = GNMDS2, label = plot, color = Year), size = 3)

# Plot GNMDS (with points and connective lines)
baseline_plot + 
  geom_point(data = gnmdstot, aes(x = GNMDS1, y = GNMDS2, color = Year, size = Year)) +
...
baseline_plot + 
  geom_text(data = gnmdstot, aes(x = GNMDS1, y = GNMDS2, label = plot, color = Year), size = 3) +
  geom_segment(data = gnmds_pairs, 
               aes(x = x1, y = y1, xend = x2, yend = y2),
               arrow = arrow(length = unit(0.2, "cm"), type = "closed"),
               color = "black")
...
# Set colors
stand_colors <- c("Tot" = "blue", "A" = "red", "B" = "green", "C" = "purple", "D" = "orange")
...
ggplot() +
  geom_point(data = displacement_data, aes(x = x1, y = y1), color = "black", size = 3) +
  geom_point(data = displacement_data, aes(x = x2, y = y2), color = "black", size = 3) +
  geom_segment(data = displacement_data, 
               aes(x = x1, y = y1, xend = x2, yend = y2, color = stand),
               arrow = arrow(length = unit(0.1, "inches"), type = "closed"), linewidth = 1) +
...
pca_sites <- scores(pca.sp, display = "sites", scaling = 1) %>% as.data.frame()
...
# Plot site scores:
pca_plot + geom_text(data = pca_sites, aes(x = PC1, y = PC2, label = labels), size = 3)


# PCA ordination of species change matrix, species score vectors 
# Choose scaling = 2
pca_species <- scores(pca.sp, display = "species", scaling = 2) %>% as.data.frame()
...
# Plot species scores:
pca_plot + geom_text(data = pca_species, aes(x = PC1, y = PC2, label = labels), size = 3)
