...
#Import from the clipboard somtimes fails, for obscure reasons (it always fails for Mac users)
...
#For import of files, remember to include te whole path to the file, or ensure that the file is placed in the working directory

#Case sensitive, breaks such as "space" not allowed, decimal sign may be changed
...
hist(Alt,nclass=10)
plot(Alt,Species)
identify(Alt,Species)

#Plot, different colours
plot(Alt,Species,type="n")
text(Alt[1:10],Species[1:10],col=3)
text(Alt[11:64],Species[11:64],col=4)


#Labelling by using "text" (here: numbers),cex
labels<-c(1:64)
plot(Alt,Species,type="n")
text(Alt,Species,labels,cex=0.7)

#Correlation test
cor.test(AvgWid,Species,method="k")
...
model1a<-glm(Species~Parea,quasipoisson)
anova(model0,model1a,test="F")
summary(model1a)	
...
#Model selection can also be execulted by an automatted procedure called 'addterm' 
...
addterm(model0,fullmodel,test="F",sorted=T)
...
#We set up a scheme for 4-fold crossvalidation by defining a factor-type vector xv that divides the
#data into four subsets:

(xv<-as.factor(rep(1:4,16)))  #note that putting the entire command in brackets implies that the result is printed on the screen
...
Null.1<-glm(Species[xv==2|xv==3|xv==4]~+1,quasipoisson)
...
Model1.2<-glm(Species[xv==1|xv==3|xv==4]~+Parea[xv==1|xv==3|xv==4],quasipoisson)
...
Model1.3<-glm(Species[xv==1|xv==2|xv==4]~+Parea[xv==1|xv==2|xv==4],quasipoisson)
...
Model1.4<-glm(Species[xv==1|xv==2|xv==3]~+Parea[xv==1|xv==2|xv==3],quasipoisson)
...
#The crossvalidation procedure can be applied to compare more complex models with the model with Parea as the only predictor
...
#Overdispersion implies that mu*(1-mu) > varm which is easily checked.

#We choose to use quasibinomial errors

model0<-glm(ComaPal~+1,quasibinomial)
summary(model0) 

#'Round 1': Testing each variable:
model1a<-glm(ComaPal~Parea,quasibinomial)
summary(model1a) 
anova(model0,model1a,test="F")
model1b<-glm(ComaPal~AvgWid,quasibinomial)
summary(model1b) 
anova(model0,model1b,test="F")
model1c<-glm(ComaPal~MaxDep,quasibinomial)
summary(model1c) 
anova(model0,model1c,test="F")
model1d<-glm(ComaPal~pH,quasibinomial)
summary(model1d) 
anova(model0,model1d,test="F")
model1e<-glm(ComaPal~Cnd,quasibinomial)
summary(model1e) 
anova(model0,model1e,test="F")
...
#Develop a 4-fold crossvalidation scheme for one of the models
