...
MR.gd <- read.geodata("P7_BSKGEO08.csv", header = TRUE, coords.col = 1:2, data.col = 3:75, sep = ";")
...
write.table(d, "d.csv")
MR.gd <- read.geodata("d.csv", header = TRUE, coords.col = 1:2, data.col = 3:75)
...
# Select a variable, for instance "Distance to the WaterTable"
sel_var <- "WatTab50"
variable_index <- which(names(MR.gd)$data == sel_var)

vario.19WatTab50.env <- variog.mc.env(MR.gd, coords = MR.gd$coords, data = MR.gd$data[, variable_index], vario.MR.gd, nsim = 999)
...
baseline_plot <- ggplot() + theme_bw() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())

# Plot semivariance
baseline_plot + 
  geom_line(data = variogram_data, aes(y = standardized_semivariance, x = breaks)) +
  geom_point(data = variogram_data, aes(y = standardized_semivariance, x = breaks), size = 2) +
  geom_line(data = variogram_data, aes(y = upper_limit, x = breaks), linetype = "dotted", color = "red") +
  geom_line(data = variogram_data, aes(y = lower_limit, x = breaks), linetype = "dotted", color = "red") +
  labs(x = "Distance", y = paste0("Standardised semivariance (", sel_var, ", upper limit)"))
...
# Creating a new plot
baseline_plot + 
  geom_line(data = variogram_data, aes(y = standardized_semivariance, x = distlog2)) +
  geom_point(data = variogram_data, aes(y = standardized_semivariance, x = distlog2), size = 2) +
  geom_line(data = variogram_data, aes(y = upper_limit, x = distlog2), linetype = "dotted", color = "red") +
  geom_line(data = variogram_data, aes(y = lower_limit, x = distlog2), linetype = "dotted", color = "red") +
  geom_vline(xintercept = 6, linetype = "dotted") + # line showing the approximate limit between within and between swamp forest scales
  geom_hline(yintercept = 1, linetype = "dotted") + # line showing semivariance = sample variance
  labs(x = "Distance (log2 scale)", y = paste0("Standardised semivariance (", sel_var, ")"))
...
variogram_data_list <- lapply(selected_variables, function(sel_var) {
  variable_index <- which(names(MR.gd)$data == sel_var)
...
baseline_plot +
  ...
  labs(x = "Distance (log2 scale)", y = paste0("Standardised semivariance"))
