...
ca.tot <- read.table("clipboard", header = TRUE)
...
# Import excel sheets
ca.tot_excel <- read_xls("P2_Oppkuven.xls", sheet = "Species") %>% as.data.frame()
ca.tot <- ca.tot_excel[, 2:ncol(ca.tot_excel)]
...
ca_sites <- scores(ca.r, display = "sites", scaling = 1) %>% as.data.frame()
...
ca_plot <- ggplot() + geom_point(data = ca_sites, aes(x = CA1, y = CA2), size = 2) +  coord_fixed(ratio = 1) + theme_bw()
...
# (2) By inserting an extra argument in the scores command like this: ca11H <- scores(ca.r, display = "sites", hill = TRUE)
# This command provides scores along the first two axes for sites, and with "species" it works for species 
# If you want to extract axes separately, this works: ca1 <- scores(ca.r, display = "sites", hill = TRUE)[, 1]
